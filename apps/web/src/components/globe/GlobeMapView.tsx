...
export function GlobeMapView({
    ...
    useEffect(() => {
        if (mapLoaded && mapInstanceRef.current && selectedPropertyId) {
            ...
            if (selectedProperty && selectedProperty.coordinates) {
                try {
                    // Set flag to prevent centerCoordinates effect and updateMarkers from interfering
                    isSelectingPropertyRef.current = true;
                    
                    // Pan to selected property with smooth animation FIRST
                    const targetCenter = {
                        lat: selectedProperty.coordinates.lat,
                        lng: selectedProperty.coordinates.lng,
                    };
                    
                    // Update previous center reference BEFORE panning to prevent other effects from interfering
                    prevCenterRef.current = { ...targetCenter };
                    
                    // Pan and zoom to selected property
                    mapInstanceRef.current.panTo(targetCenter);
                    mapInstanceRef.current.setZoom(15);
                    
                    // Update markers AFTER panning to ensure correct markers are shown
                    // Use a small delay to let the pan complete
                    setTimeout(() => {
                    ...
                    // Open info window for selected property after marker is created
                    setTimeout(() => {
                    ...
                    }, 500); // Increased delay to ensure marker is created
                    ...
                    zoomListenerRef.current = mapInstanceRef.current.addListener('zoom_changed', () => {
                        if (mapInstanceRef.current) {
                            ...
                            if (currentZoom <= 6) {
                                ...
                                // Use debounce to prevent too frequent updates that interfere with user zoom
                                if (!selectedPropertyId && !isSelectingPropertyRef.current && mapInstanceRef.current) {
                                    ...
                                    zoomUpdateTimeoutRef.current = setTimeout(() => {
                                        if (mapInstanceRef.current && !selectedPropertyId && !isSelectingPropertyRef.current) {
                                            ...
                                            if (Math.abs(currentZoomAtUpdate - zoomAtStart) < 0.5) {
                                                updateMarkers();
                                            }
                                        }
                                        zoomUpdateTimeoutRef.current = null;
    ...
    const updateMarkers = () => {
        ...
        // DON'T auto-fit or reset zoom/center in updateMarkers
        // This function should ONLY update markers, not change the map view
        // Map view changes should be handled by:
        // 1. Property selection effect (for selectedPropertyId)
        // 2. Center coordinates effect (for external center changes)
        // 3. User interaction (zoom, pan)
        // This prevents the map from resetting when user zooms out
    };
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between p-6 border-b border-white/10">
                                    <div>
                                        ...
                                        <p className="text-sm text-mist-400">
                                            {(() => {
                                                let filteredCount = selectedCluster.properties.length;
                                                if (clusterSearchQuery.trim()) {
                                                    const query = clusterSearchQuery.toLowerCase().trim();
                                                    filteredCount = selectedCluster.properties.filter(prop => 
                                                        prop.location.toLowerCase().includes(query) ||
                                                        (prop.name && prop.name.toLowerCase().includes(query))
                                                    ).length;
                                                }
                                                if (clusterPriceFilter !== 'all') {
                                                    const priceFiltered = selectedCluster.properties.filter(prop => {
                                                        const price = prop.price || 0;
                                                        switch (clusterPriceFilter) {
                                                            case '0-100':
                                                    ...
                                                    if (clusterSearchQuery.trim()) {
                                                        const query = clusterSearchQuery.toLowerCase().trim();
                                                        filteredCount = priceFiltered.filter(prop => 
                                                            prop.location.toLowerCase().includes(query) ||
                                                            (prop.name && prop.name.toLowerCase().includes(query))
                                                        ).length;
                                                    } else {
                                                ...
                                                return filteredCount === selectedCluster.count 
                                                    ? 'Select a property to view details' 
                                                    : `Showing ${filteredCount} of ${selectedCluster.count} properties`;
                                            })()}
                                        </p>
                                    </div>
                                    <button
                                ...
                                <div className="p-6 border-b border-white/10 space-y-4">
                                    ...
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full px-4 py-3 pl-10 bg-white/5 border border-white/20 rounded-lg text-mist-100 placeholder-mist-400 focus:outline-none focus:ring-2 focus:ring-earth-500 focus:border-earth-500 transition-all"
                                        />
                                        <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-mist-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                        </svg>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <label className="text-sm text-mist-400">Price Range:</label>
                                        <select
                                            value={clusterPriceFilter}
                                            onChange={(e) => setClusterPriceFilter(e.target.value)}
                                            className="px-3 py-2 bg-white/5 border border-white/20 rounded-lg text-mist-100 focus:outline-none focus:ring-2 focus:ring-earth-500 focus:border-earth-500 transition-all"
                                        >
                                ...
                                <div className="overflow-y-auto max-h-[60vh] p-6">
                                    {(() => {
                                        ...
                                        let filteredProperties = selectedCluster.properties;
                                            ...
                                            <>
                                                {filteredProperties.length === 0 ? (
                                                    <div className="text-center py-12">
                                                        <p className="text-mist-400 text-lg">No properties found matching your criteria</p>
                                                        <button
                                                            onClick={() => {
                                                                setClusterSearchQuery('');
                                                                setClusterPriceFilter('all');
                                                            }}
                                                            className="mt-4 text-earth-500 hover:text-earth-400 transition-colors text-sm"
                                                        >
                                                            Clear filters
                                                        </button>
                                                    </div>
