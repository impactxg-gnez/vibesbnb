          ...
          if (propertyData) {
            ...
            if (propertyData.rooms && Array.isArray(propertyData.rooms) && propertyData.rooms.length > 0) {
              const loadedRooms: Room[] = propertyData.rooms.map((r: any, idx: number) => ({
                id: r.id || `room-${idx}-${Date.now()}`,
                name: r.name || `Unit ${idx + 1}`,
                price: r.price || loadedProperty.price || 100,
                guests: r.guests || loadedProperty.guests || 2,
                images: [],
                imagePreviewUrls: r.images || [],
              }));
              setRooms(loadedRooms);
              // Check if this is a multi-unit property (has price info per unit or multiple rooms)
              const hasUnitPricing = propertyData.rooms.some((r: any) => r.price !== undefined);
              setIsMultiUnit(propertyData.rooms.length > 1 || hasUnitPricing);
              ...
              setRooms([
                {
                  ...
                  name: 'All Photos',
                  price: loadedProperty.price || 100,
                  guests: loadedProperty.guests || 2,
                  images: [],
                  imagePreviewUrls: propertyData.images || [],
                },
              ]);
          ...
          try {
            ...
            if (property) {
              ...
              if (property.rooms && Array.isArray(property.rooms) && property.rooms.length > 0) {
                ...
                const hasUnitPricing = property.rooms.some((r: any) => r.price !== undefined);
                setIsMultiUnit(property.rooms.length > 1 || hasUnitPricing);
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
            ...
            <button
              ...
              className={`w-full md:w-auto px-4 py-3 rounded-lg border transition flex items-center justify-center gap-2 ${
                formData.wellnessFriendly
