...
export class SeedController {
  ...
  async runSeed() {
    try {
      ...
      const adminId = await this.firebase.create('users', {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        const hostId = await this.firebase.create('users', {
      ...
      for (let i = 1; i <= 5; i++) {
        const passwordHash = await bcrypt.hash('password123', 10);
        const guestId = await this.firebase.create('users', {
      ...
      for (const host of hosts) {
        for (let i = 0; i < 4; i++) {
          ...
          await this.firebase.create('calendars', {
            listingId,
            source: 'internal',
          ...
          // Add mock images
          for (let j = 0; j < 5; j++) {
            await this.firebase.create('listing_media', {
              listingId,
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const daysFromNow = (i % 3 === 0) ? -30 : (i % 3 === 1) ? 7 : 30;
        ...
        if (status === BookingStatus.CHECKED_OUT) {
          await this.firebase.create('reviews', {
            bookingId,
            authorId: guest.id,
      ...
      for (let i = 0; i < 10; i++) {
        ...
        // Add messages
        await this.firebase.create('messages', {
          threadId,
          senderId: guest.id,
      ...
      return {
        ...
        data: {
          users: { admin: 1, hosts: 3, guests: 5 },
          listings: 12,
          bookings: 20,
          messages: 10,
          testAccounts: {
            admin: { email: 'admin@vibesbnb.com', password: 'admin123' },
            host: { email: 'host1@vibesbnb.com', password: 'password123' },
            guest: { email: 'guest1@vibesbnb.com', password: 'password123' },
          },
        },
      };
    } catch (error) {
